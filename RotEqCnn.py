...
class RotEqCNN():
    def __init__(self, ensemble_num=1, dataset='mnist', train_epoch=1): # change the ensemble_number and train_epoch
    ...
    def train(self):
            ...
            with torch.no_grad():
                for data in self.trainloader[i]:
                    ...
                    for j in range(len(outputs[0])):
                        ...
                        print("The data is: " + str(d) + " lable is: " + str(labels[j]) + "\n")
        X_train, self.X_test, y_train, self.y_test = train_test_split(self.encoded_train_data, self.encoded_train_label,
         ...
         #use sklearn simple version
         #self.mlp = MLPClassifier(max_iter=200,hidden_layer_sizes = (100))
                 # Consider to change to something like this (Jan/25/2021)
                 # multiple layer neural network could be better
                 # self.mlp = MLPClassifier(max_iter=200,hidden_layer_sizes = (64, 64))
         #self.mlp.fit(X_train, y_train)
        

        np.savetxt('encoded_train_data.txt',self.encoded_train_data, fmt='%.8f')
        np.savetxt('encoded_train_label.txt', self.encoded_train_label, fmt='%d')
        #print(self.model)
        ...
        #self.mlp = ES_clf(k = self.ensemble_num * 10).to(self.device)
        #        # Consider to change to something like this (Jan/25/2021)
        #        # multiple layer neural network could be better
        #        # self.mlp = MLPClassifier(max_iter=200,hidden_layer_sizes = (64, 64))
        #self.train_es_clf(X_train, y_train)
        ...
        #else:
        #   read_train_data = np.loadtxt('X_test.txt', dtype=float)
        #    X_train, self.X_test, y_train, self.y_test = train_test_split(read_train_data, self.encoded_train_label,
        #                                                              test_size=0.01, random_state=0)
        #    # transform data into tensors
        #    X_train = torch.FloatTensor(X_train)
        #    print(X_train.shape)
    ...
    def train_es_clf(self):
        print("read data set")
        read_train_data = np.loadtxt('encoded_train_data.txt', dtype=float)
        read_label = np.loadtxt('encoded_train_label.txt',dtype=int)
        X_train, self.X_test, y_train, self.y_test = train_test_split(read_train_data, read_label,
                                                                      test_size=0.01, random_state=0)
        # transform data into tensors
        X_train = torch.FloatTensor(X_train)
        print(X_train.shape)
    ...
    def predict_version1(self, image, label):
        ...
        for i in range(len(test_encoded_set[0])):
            ...
            for j in range(self.ensemble_num):
                ...
                op=d
        # pred = self.mlp.predict([d])
        d = np.array(d)
        d = d.reshape(1, self.ensemble_num*10)
        d = torch.FloatTensor(d).cuda()
        self.mlp.eval()
        pred = self.mlp(d)
        prediction = torch.max(F.softmax(pred), 1)[1]
        print("prediction" + str(prediction) + "\n")
        print("prediction two" + str(torch.max(F.softmax(pred), 1)) + "\n")
        print("prediction three" + str(torch.max(F.softmax(pred), 0)) + "\n")
        print("prediction four" + str(pred) )
        print(pred.shape)
